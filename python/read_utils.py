...
def estimate_cost_matrix(gt_labels, cluster_labels):
    ...
    if len(gt_labels) != len(cluster_labels):
        print('The dimensions of the gt_labels and the pred_labels do not match')
        # Adjust the length of cluster_labels to match gt_labels
        if len(cluster_labels) > len(gt_labels):
            cluster_labels = cluster_labels[:len(gt_labels)]
        else:  # len(cluster_labels) < len(gt_labels)
            cluster_labels = np.concatenate([cluster_labels, [cluster_labels[-1]]] * (len(gt_labels) - len(cluster_labels)))
